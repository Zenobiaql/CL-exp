...
def compare_tensors(tensors_0, tensors_1):
    ...
    for key in tqdm(key_pairs):
        ...
        if "vision" in key:
            num_vision_layers += 1
            #print(f"key: {key}")
            tensor_0 = (torch.t(tensors_0[key+".lora_A.weight"])@torch.t(tensors_0[key+".lora_B.weight"])).to(torch.float32)
            tensor_1 = (torch.t(tensors_1[key+".lora_A.weight"])@torch.t(tensors_1[key+".lora_B.weight"])).to(torch.float32)
            #print(f"tensor_0: {tensor_0.shape}, tensor_1: {tensor_1.shape}")
            cosine_sim = cosine_similarity(tensor_0, tensor_1)
